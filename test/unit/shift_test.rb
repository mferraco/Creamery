...
class ShiftTest < ActiveSupport::TestCase
  ...
  context "Creating a context" do
    ...
    should "return correct shift length" do
      @shift1 = FactoryGirl.build(:shift, :assignment => @assign_cindy, :start_time => Time.local(2012,1,1,23,0,0), :end_time => Time.local(2012,1,1,2,0,0))
      @shift2 = FactoryGirl.build(:shift, :assignment => @assign_ben, :start_time => Time.local(2012,1,1,9,0,0), :end_time => Time.local(2012,1,1,12,0,0))
      
      assert_equal @shift2.get_shift_length, 3
      assert_equal @shift1.get_shift_length, 3
    end
